...
def dashboard_view(request):
    ...
    recent_daily_records = DailyRecord.objects.filter(
        batch__farm__owner=request.user
    ).select_related(
        "batch",
        "batch__farm",
    ...
    recent_egg_productions = EggProduction.objects.filter(
        batch__farm__owner=request.user
    ).select_related(
        "batch",
        "batch__farm",
...
def batch_detail_view(request, batch_id):
    ...
    egg_productions = EggProduction.objects.filter(
        batch=batch
    ).order_by(
        "-production_date",
        "-id",
...
@login_required
def egg_production_edit_view(request, production_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "batch_detail",
                batch_id=egg_production.batch.id,
            )
...
@login_required
def egg_production_delete_view(request, production_id):
